...
if (ECONOMY_MODE >= 1) exitWith {
    //Geld-Umbau Schritt 3: Wert, Belohnung und Leeren der Kiste uebernimmt der Server
    ["TON_fnc_econIncome", ["seize", _container, "container"], {}, {
        [ localize "STR_Cop_NoIllegalContainer",false,"fast"] call life_fnc_notification_system;
    }] call life_fnc_econRequest;
};
if (_value > 0) then {
    ["life_fnc_broadcast",[0,"STR_NOTF_ContainerContraband",true,[[_value] call life_fnc_numberText]],RCLIENT] call life_fnc_relaySend;
    BANK = BANK + _value;
